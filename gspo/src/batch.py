# coding=utf-8
"""WeDLM batch construction — self-contained for gspo/."""
...
class WeDLMBatch:
    ...
    Mirrors dpo.src.batch.WeDLMBatch — self-contained copy for gspo/.
    """
...
def build_wedlm_batch(
    ...
    Given packed sequences (input_ids + labels as prompt mask), produces
    the x0/xt dual-stream representation used by WeDLM attention.

    Args:
        packed_input_ids: Flattened token ids [T] across all sequences.
        packed_labels: Flattened labels [T] where -100 marks prompt (never masked).
        cum_seqlens: Cumulative sequence lengths [bs+1] over T.
        block_size: WeDLM block size B.
        mask_token_id: ID of the [MASK] token.
        mask_per_block: If True, each block gets an independent masking ratio γ ~ U(0,1).
        backend: "magi" or "dense".
        eps: Floor for masking ratio.

    Returns:
        WeDLMBatch with x0/xt packed representation and attention plan.
    """
...
def build_wedlm_batch_from_response(
    ...
    """Build a WeDLM batch from a single response (prompt + completion).

    This is a convenience wrapper used during GSPO training when scoring
    generated responses — the prompt tokens are never masked, and the
    completion tokens are randomly masked per WeDLM training convention.

    Args:
        response_ids: Full token ids [L] = prompt_ids + completion_ids.
        prompt_len: Length of the prompt prefix (these tokens are never masked).
        block_size: WeDLM block size.
        mask_token_id: ID of the [MASK] token.
        backend: "magi" or "dense".
        mask_per_block: Per-block independent masking ratios.
        eps: Floor for masking ratio.

    Returns:
        WeDLMBatch ready for wedlm_forward.
    """
    device = response_ids.device
    L = response_ids.size(0)

    labels = response_ids.clone()
    labels[:prompt_len] = -100  # prompt never contributes to MLM target

    cum_seqlens = torch.tensor([0, L], device=device, dtype=torch.long)

    return build_wedlm_batch(
        packed_input_ids=response_ids,
        packed_labels=labels,
        cum_seqlens=cum_seqlens,
        block_size=block_size,
        mask_token_id=mask_token_id,
        mask_per_block=mask_per_block,
        backend=backend,
        eps=eps,
    )
