...
from src.data import WeDLMPackedDataset, WeDLMPairwiseDataset, packed_collate_fn, dpo_collate_fn, get_im_end_token_id
from src.batch import WeDLMBatch, build_wedlm_batch
from src.model import wedlm_forward
from src.loss import compute_mlm_loss, compute_ar_loss, compute_block_scores
...
class WeDLMTrainer:
    ...
    def _setup(self):
        ...
        if self.config.training_mode == "dpo":
            ...
            self.ref_model.eval()
            logger.info(f"Loaded DPO reference model from {ref_model_path}")
    ...
    def _prepare_training(self):
        ...
        if self.config.training_mode == "dpo" and self.ref_model is not None:
            try:
